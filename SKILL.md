...
**같은 계정으로 hancom.js 를 동시에 두 개 이상 돌리면 안 된다.** 한컴독스는 동일 계정 동시 다중 로그인을 보안 위반으로 보고 **모든 세션을 로그아웃시키고 재로그인을 차단** → **비밀번호를 바꿔야 복구**된다. 브라우저를 띄우는 모든 명령(capture·around·편집·trash 등)은 로그인을 하므로, 보기 전용이라도 겹치면 잠긴다. **항상 순차 실행** — 한 명령이 끝난 뒤 다음을 시작.

이를 강제하려고 **세션 락이 자동으로 걸린다**(별도 조작 불필요):
- 브라우저 명령을 시작하면 락 파일(`<os.tmpdir>/hancom-session.lock`)을 잡고, 끝나면 푼다. 이미 활성 세션이 있으면 새 명령은 **즉시 거부**(`{"status":"session_busy",...}`, exit 7) — 브라우저를 안 띄우니 이중 로그인이 원천 차단된다. (락은 머신 공유 경로라 같은 계정으로 로그인하는 **다른 도구·다른 repo**(예: 캡처 전용 플러그인)와도 공유돼 캡처↔편집 충돌까지 막는다. 프로세스가 죽었거나 20분 넘게 묵은 락은 stale 로 자동 회수.)
...
- **capture**: 파일을 (필요시) 업로드하고 N쪽(기본 1)을 **A4 한 장 깔끔히** 캡처(툴바·여백 없음, 잘림 없음). 반환 `{shot, docName, page, totalPages, estTotalPages, pageWidth, pageHeight}`.
  - **`totalPages`**: 상태바에서 읽은 총 쪽수(가시 상태요소 전체에서 `현재/총` 후보 중 total 최댓값 채택 — 일부 문서의 stale `1/1` 오판 방지). 못 읽으면 `null`이고 `estTotalPages`가 스크롤 기반 추정으로 폴백. ⚠️ **대용량/지연 페이지네이션 문서는 로드 직후 총쪽수가 1로 적게 나올 수 있다**(상태바가 아직 안 풀림) — 정확한 위치 잡기는 `pinpoint`/`around`(검색 기반)가 신뢰성 높다.
...
- **`trash`** (드라이브 문서 삭제): 드라이브 문서를 **휴지통으로 이동**(영구삭제 아님 — 한컴독스 웹 **휴지통** 탭에서 복구/영구삭제, 직후엔 '실행 취소' 스낵바). 주로 테스트로 올린 문서 정리에 쓴다.
  ...
  - 결과의 `results[].status`: `trashed`(이동됨) · `not_found` · `delete_disabled`(**그 문서가 아직 편집 세션에 열려 있음** — 한컴독스는 열려 있는 문서의 '삭제'를 비활성화한다. 방금 편집한 문서는 서버 presence가 잠깐 남아 열린 상태로 잡히므로, 그 세션이 닫힌 뒤 재시도) · `maybe_failed`(이동 확인 실패 → 재시도).
...
- **`chart-style --at "x,y"`**: 차트의 **모양·색 테마**를 바꾼다(차트를 더블클릭해 편집모드로 들어가 적용). `--at` 은 차트 중앙(먼저 `find-objects` 로 확인).
  ...
  - **`--theme N`** — **색 팔레트**(1~ 인덱스; 팔레트 묶음의 N번째). 차트 전체 색이 그 팔레트로 한 번에 바뀐다. 팔레트별 정확한 색은 문서마다 다르니, **원하는 색이 나올 때까지 N을 바꿔 가며 줌 캡처로 확인**(아래 검증 참고).
  ...
  - ⚠️ **`saved:false`/`save_unconfirmed`이 떠도 실제론 적용됐을 수 있다** — 차트 편집은 저장 신호가 늦게 잡혀 미확인으로 뜰 때가 있다. **무조건 재오픈(`zoom`/`around`)해 색이 바뀌었는지 확인**할 것(아래).
  - **색 검증 = 줌 캡처**: `find-objects` 로 차트 중앙 좌표를 얻어 `zoom --clip "x,y,w,h" --scale 3` 으로 막대(플롯) 영역을 크게 찍어 색을 본다. (차트 **색**은 캡처가 진실 — 선 종류 같은 swap 문제 없음. 정밀 검증이 필요하면 `download` 후 `Chart/chart1.xml` 의 `schemeClr/srgbClr` 확인.)
